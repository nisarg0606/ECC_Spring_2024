...
exec > >(tee -i cmd_grep_$(date '+%Y-%m-%d-%H-%M').log)
exec 2>&1

# Define HDFS input and output paths
hdfs_input="/Input/access.log"
hdfs_output="/assignment1/Grep_Output/grep_output_$(date '+%Y-%m-%d-%H-%M')"
...
output_file="/home/hadoop/ECC_Spring_2024/assignment1/output_grep_$(date '+%Y-%m-%d-%H-%M').txt"
