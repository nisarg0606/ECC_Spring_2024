...
exec > >(tee -i cmd_sort_$(date '+%Y-%m-%d-%H-%M').log)
exec 2>&1

# Define HDFS input and output paths
hdfs_input="/Input/examples.txt"
hdfs_output="/assignment1/Sort_Output/sort_output_$(date '+%Y-%m-%d-%H-%M')"
...
output_file="/home/hadoop/ECC_Spring_2024/assignment1/output_sort_$(date '+%Y-%m-%d-%H-%M').txt"
